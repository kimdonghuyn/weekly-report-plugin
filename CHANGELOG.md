...
- **Drafts 파일 지원**: Figma의 Drafts(내 초안) 파일은 팀 프로젝트 API에 잡히지 않는
  문제를 실사용 검증에서 발견, `figma.fileKeys` 설정으로 파일 키를 직접 지정해 스캔할
  수 있게 추가 (`"키"` 문자열 또는 `{ "key", "name" }` 객체). 팀 스캔에서 이미 다룬
  파일은 중복 조회하지 않는다.
- 실계정 end-to-end 검증 완료: 실제 토큰·팀·파일·이름 붙인 버전이 보고서 데이터로
  정확히 수집되는 것을 확인.

## 1.4.0 — 2026-08-07

- **Figma 연동(옵션)**: `config.json`의 `figma` 섹션(`token`·`teamIds`·`userHandles`)을
  채우면 그 주에 작업된 Figma 파일의 버전 히스토리를 스캔해 결과 JSON의 `figma` 배열로
  반환하고, 보고서에 "디자인 (Figma)" 섹션이 추가된다. 토큰은 `FIGMA_TOKEN` 환경변수로도
  설정 가능. 버전마다 작성자(handle)가 붙으므로 실행자 토큰 하나로 팀 전체 활동을
  사람별로 구분해 집계할 수 있다.
- 모든 데이터 소스가 옵션이 되도록 정리: `needsSetup`은 이제 `scanRoots`와 `figma`가
  모두 비어 있을 때만 `true`다. git 없이 Figma + 수동 기록만으로도 보고서 생성 가능.
- 그 주에 수정되지 않은 파일은 버전 조회를 건너뛰고, 버전 페이지네이션은 주 시작 이전에
  도달하면 중단한다. 조회 실패(권한/rate limit)는 해당 팀/파일만 건너뛴다.
